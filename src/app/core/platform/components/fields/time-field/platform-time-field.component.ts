...
import { ComponentFieldType, TimeFieldConfig } from '../../component.types';

@Component({
  selector: 'platform-time-field',
  standalone: true,
    ...
    >
      @if (skeleton()) {
        ...
        <mat-form-field class="pf-form-field" appearance="outline">
          ...
          <mat-icon matPrefix aria-hidden="true">schedule</mat-icon>
          <input
            matInput
            [type]="inputType()"
            [id]="fieldId()"
            ...
            [disabled]="isDisabled()"
            [readOnly]="readonly()"
            [placeholder]="placeholder() || (is24h() ? 'HH:MM' : 'HH:MM AM/PM')"
...
export class PlatformTimeFieldComponent extends BaseFieldComponent<string> {
  override readonly componentKey = 'platform-time-field';
  override readonly fieldType: ComponentFieldType = 'time';

  protected is24h = computed(() => (this.config() as TimeFieldConfig).format !== '12h');

  protected inputType = computed(() => 'time');
}
