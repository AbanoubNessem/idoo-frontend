...
import { ComponentFieldType, CurrencyFieldConfig } from '../../component.types';

@Component({
  selector: 'platform-currency-field',
  standalone: true,
    ...
    >
      @if (skeleton()) {
        ...
        <mat-form-field class="pf-form-field" appearance="outline">
          ...
          @if (currencySymbol()) {
            <span matTextPrefix class="pf-currency-prefix">{{ currencySymbol() }}&nbsp;</span>
          } @else if (prefixIcon()) {
          ...
          <input
            ...
            [attr.min]="currConfig().min ?? null"
            [attr.max]="currConfig().max ?? null"
            [attr.step]="currConfig().step ?? 0.01"
  ...
  styles: [`
    ...
    .pf-currency-prefix { color: var(--platform-color-text-secondary, #64748b); font-weight: 500; }
    input[type=number]::-webkit-inner-spin-button,
    input[type=number]::-webkit-outer-spin-button { -webkit-appearance: none; margin: 0; }
    input[type=number] { -moz-appearance: textfield; }
  `],
})
export class PlatformCurrencyFieldComponent extends BaseFieldComponent<number> {
  override readonly componentKey = 'platform-currency-field';
  override readonly fieldType: ComponentFieldType = 'currency';

  protected currConfig(): CurrencyFieldConfig {
    return this.config() as CurrencyFieldConfig;
  }

  protected currencySymbol = computed(() => {
    const cfg = this.currConfig();
    if (!cfg.showSymbol && cfg.showSymbol !== undefined) return '';
    const currency = cfg.currency ?? 'USD';
    const locale   = cfg.locale ?? this.ctx.locale();
    try {
