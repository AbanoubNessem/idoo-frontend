...
@Component({
  selector: 'platform-checkbox-field',
  standalone: true,
    ...
    >
      @if (skeleton()) {
        <div class="pf-skeleton-wrap" role="status" aria-label="Loading field">
          <div class="pf-skeleton-checkbox-row">
            <div class="pf-skeleton-box"></div>
            <div class="pf-skeleton-label"></div>
          </div>
        </div>
      } @else {
        <div class="pf-checkbox-container">
          @if (loading()) {
          ...
          @for (e of errors(); track $index) {
            <div class="pf-error" [attr.id]="$index === 0 ? errorId() : null" role="alert">{{ e }}</div>
          }
        </div>
      }
    </div>
  ...
  styles: [`
    :host { display: block; }
    .pf-checkbox-container { display: flex; flex-direction: column; gap: 4px; }
    ...
    .pf-skeleton-checkbox-row { display: flex; align-items: center; gap: 12px; }
    .pf-skeleton-box { width: 18px; height: 18px; border-radius: 3px; background: var(--platform-color-surface-variant, #e2e8f0); animation: pf-shimmer 1.5s infinite linear; flex-shrink: 0; }
    .pf-skeleton-label { height: 14px; width: 120px; border-radius: 4px; background: var(--platform-color-surface-variant, #e2e8f0); animation: pf-shimmer 1.5s 0.1s infinite linear; }
    @keyframes pf-shimmer { 0% { opacity: 1; } 50% { opacity: 0.5; } 100% { opacity: 1; } }
  `],
})
export class PlatformCheckboxFieldComponent extends BaseFieldComponent<boolean> {
  override readonly componentKey = 'platform-checkbox-field';
  override readonly fieldType: ComponentFieldType = 'checkbox';

  protected onCheckChange(event: { checked: boolean }): void {
