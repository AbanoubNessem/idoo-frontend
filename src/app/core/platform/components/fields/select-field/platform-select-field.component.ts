...
import { ComponentFieldType, SelectFieldConfig, SelectOption } from '../../component.types';

@Component({
  selector: 'platform-select-field',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [MatFormFieldModule, MatSelectModule, MatIconModule, MatProgressSpinnerModule],
  template: `
    <div
      class="pf-field-host"
...
export class PlatformSelectFieldComponent extends BaseFieldComponent<unknown> {
  override readonly componentKey = 'platform-select-field';
  override readonly fieldType: ComponentFieldType = 'select';

  protected selectConfig(): SelectFieldConfig {
    return this.config() as unknown as SelectFieldConfig;
  }

  protected options = computed<SelectOption[]>(() => this.selectConfig().options ?? []);
}
